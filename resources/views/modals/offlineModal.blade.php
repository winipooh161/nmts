...
<div class="modal" id="offlineModal" style="display: none;">
    <div class="modal__body">
        ...
        <div class="modal__form offlineModal">
            <form id="offlineForm">
                @csrf
                <h6 class="tt">Придёшь офлайн?</h6>
                <p>Мы вышлем подробную информацию на твою почтау</p>
            </form>
            <label for="" class="yes__button__no">
                <button type="button" value="Да" class="open-modal-btn close-modal" data-modal="offlineModalYES" id="offline-yes-btn">{{ __('Да') }}</button>
                <a href="#" class="open-modal-btn close-modal" id="offline-no-btn" value="Нет">{{ __('Нет') }}</a>
            </label>
        </div>
    </div>
</div>
...
<script>
    // Функция отправки значения оффлайн через AJAX
    function sendOfflineStatus(status) {
        axios.post('{{ route('offlineModal') }}', {
            _token: '{{ csrf_token() }}',
            offline: status
        })
        .then(function (response) {
            console.log(response.data);
            document.getElementById('BtnModalNone').style.display = 'none';
            document.getElementById('offlineModal').style.display = 'none';
        })
        .catch(function (error) { });
    }
    // Обработчики кликов для кнопок
    document.getElementById('offline-yes-btn').addEventListener('click', function() {
        sendOfflineStatus('Да');
    });
    document.getElementById('offline-no-btn').addEventListener('click', function() {
        sendOfflineStatus('Нет');
    });
</script>
@endsection
