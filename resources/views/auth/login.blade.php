    ...
    <div class="container register__fon">
        <div class="form__auth login">
            ...
            <form method="POST" action="{{ route('login') }}">
                @csrf
                <div class="name__user__mail__input pd20nottom">
                    <label for="email">
                        <p>{{ __('Корпоративная почта') }}</p>
                    <input id="email" type="email" class="form-control @error('email') is-invalid @enderror"
                        name="email" value="{{ old('email') }}" placeholder="Введи почту" required maxlength="70"
                        autocomplete="email">
                    @error('email')
